...
`Taxonomy`.  This must be with respect to some `Applicability`.
...
[TIP]
====
The (non-ASF) http://github.com/incodehq/incode-module-alias[Incode alias] module also allows the title/icon/CSS class to be overridden; its example application includes an example to demonstrate how this is done.
====
