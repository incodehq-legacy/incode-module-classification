...
total about link:https://github.com/incodehq/incode-module-classification/blob/master/fixture/src/main/java/org/incode/module/classification/fixture/app/classification/demo/ClassificationForDemoObject.java[50 lines of boilerplate] are required, details below.
...
applicable for domain objects at that leve and sub-levels (eg "/ITA/MIL").  The value of available the application
...
There can be many such taxonomies; any given domain object can only have one `Classification` per `Taxonomy`.
...
Not every domain object can be classified with respect to every ``Taxonomy``.  Instead, the available set is restricted by
the `Applicability` entity.  This identifies which ``Taxonomy``(s) can be associated with domain object types.
This is further qualified by the application tenancy of the domain object (for multi-tenanted applications).  The
`ApplicationTenancyService` SPI service is used to obtain the application tenancy of each domain object.
...
image::https://raw.githubusercontent.com/incodehq/incode-module-classification/master/images/030-list-taxonomies[link="https://raw.githubusercontent.com/incodehq/incode-module-classification/master/images/030-list-taxonomies.png"]
...
In contrast to the two "colour" taxonomies, the "Size" taxonomy is defined globally (for the "/" app tenancy).  It does
only apply to the `DemoObject` domain type, not to the `OtherObject` domain type.
...
"classified" domain object, and then as the actual subtype when the alias object itself is shown (eg
...
(ie link:https://github.com/incodehq/incode-module-classification/blob/master/fixture/src/main/java/org/incode/module/classification/fixture/app/classification/ClassificationForDemoObject.layout.xml[`ClassificationForDemoObject.layout.xml`])
...
[TIP]
====
The (non-ASF) http://github.com/incodehq/incode-module-alias[Isis addons' alias] module also allows the title/icon/CSS class to be overridden; its example application includes an example to demonstrate how this is done.
====
